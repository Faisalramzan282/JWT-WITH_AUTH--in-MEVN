  ...
  <div class="bg-gray-100">
    ...
    <nav class="bg-blue-500 p-4">
      <div class="container mx-auto flex justify-evenly items-center">
        ...
        <button @click="isPasswordPopUp = true" class="text-white p-2 rounded-md hover:bg-stone-500 ">Update
          Profile</button>
        ...
        <input type="text" class="w-64 px-2 py-1 border rounded focus:outline-none focus:border-blue-300"
    ...
    <div class="container mx-auto p-8">
      <form @submit.prevent="submitForm" class="max-w-md mx-auto bg-white rounded shadow p-6">
        ...
        <input type="text" id="movieName" v-model="moviesData.movieName" required
        ...
        <input type="date" id="releaseDate" v-model="moviesData.releaseDate" required
          class="w-full p-2 border rounded focus:outline-none focus:border-blue-400" />
        <br />
        <label for="movieTickets" class="block font-semibold mb-2">Total Tickets</label>
        <input type="number" id="movieTickets" v-model="moviesData.tickets" required
          class="w-full p-2 border rounded focus:outline-none focus:border-blue-400" />
        <label for="moviePrice" class="block font-semibold mb-2">Price</label>
        <input type="number" id="moviePrice" v-model="moviesData.moviePrice" required
          class="w-full p-2 border rounded focus:outline-none focus:border-blue-400" />
        <button type="submit"
          class="mt-4 bg-blue-500 text-white p-2 rounded-md hover:bg-blue-600 focus:outline-none focus:bg-blue-600">Submit</button>
      </form>
    </div>
    <!-- Movie List -->
    <div class="container mx-auto p-8">
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
        <div v-for="movie in fetchMovies" :key="movie._id" class="max-w-sm mx-auto bg-white rounded shadow p-6">
          ...
          <div class="mt-4">
            <button @click="showEditMoviesPopup(movie)"
              class="p-2 bg-slate-300 text-white rounded-md hover:bg-blue-600 focus:outline-none">Edit</button>
            
            <EditMoviesPopUp v-if="isPopupVisible" :movieDetail='selectedMovie' @close="closePopup" />
            <button @click="deleteMovie(movie)"
              class="p-2 bg-slate-300 text-white rounded-md bg-red-400 focus:outline-none">Delete</button>
          </div>
        </div>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...mapActions({ moviesCreation: "movies/moviesCreation", getMovies: "movies/getMovies", deleteMov: "movies/deleteMov", delete_User: "movies/delete_User", role_selection: "movies/role_selection" }),
    async submitForm() {
